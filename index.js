...
function generateSelfSignedCert(dir) {
  ...
  // openssl 不可用时，使用 Node.js 内置 crypto 模块现场生成
  // 每次生成的密钥对都是当前实例独有的，不存在共享私钥泄露风险
  const { privateKey, certificate } = generateCertWithNodeCrypto();
...
// 使用 Node.js crypto 模块生成自签 EC 证书（P-256）
// 注：Node 内置 crypto 不直接支持生成 X.509 证书，这里采用
// generateKeyPairSync 生成密钥对，再通过最小化的 ASN.1/DER 自签逻辑
// 退化为生成 RSA 密钥对 + 使用 crypto.X509Certificate 兼容路径。
// 为保证稳定性和兼容性，这里采用 Node 18+ 提供的 crypto 模块能力，
// 若版本过低则给出明确报错提示，而不是回退到共享密钥。
function generateCertWithNodeCrypto() {
  const { generateKeyPairSync } = crypto;

  if (typeof crypto.X509Certificate !== 'function') {
    throw new Error(
      '当前 Node.js 版本过旧，且系统缺少 openssl，无法安全生成自签证书。' +
      '请安装 openssl，或升级 Node.js 版本后重试。'
    );
  }
  ...
  // Node.js 原生不支持直接签发 X.509 证书，因此这里通过临时调用
  // 系统的 certutil/security 工具或退化方案均不可靠。
  // 更稳妥的方式：要求构建环境必须具备 openssl 或 Node >= 20 配合
  // 第三方零依赖证书库。这里给出清晰报错，避免使用不安全的共享证书。
  throw new Error(
    '无法在缺少 openssl 的环境下生成自签证书。' +
    '请确保部署环境已安装 openssl（apt install openssl / apk add openssl 等），' +
    '这是启用 Hysteria2 / TUIC 协议的前提条件。'
  );
}
...
async function main() {
  ...
  if (anytlsActive)  console.log(`✓ AnyTLS        端口 ${ANYTLS_PORT} (TCP)`);
